	...
	for generation := 0; generation < GENERATION_COUNT; generation++ {
		...
		for i := 0; i < channelsCount; i++ {
			...
			fmt.Println(fmt.Sprintf(
				"Gen: %d, Bot: %d, Buys Count: %d, Success: %d, Failed: %d, Revenue: %f, SuccessPercentage: %f, Selection: %f\n",
				generation,
				botRevenue.BotNumber,
				botRevenue.TotalBuysCount,
				botRevenue.SuccessBuysCount,
				botRevenue.FailedBuysCount,
				rev,
				successPercentage,
				CalcSelection(rev, successPercentage)))
		}
