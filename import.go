...
func ImportFromCsv(fileName string) []BotConfig {
	...
	for rowNumber, row := range rows {
		...
		bot := BotConfig{
			...
			WaitAfterPeriod:     convertStringToInt(row[73]),
			WaitAfterMinRevenue: convertStringToFloat64(row[74]),

			MinQuoteVolume: convertStringToFloat64(row[75]),

			TrailingIncreaseSpeedCoefficient: convertStringToFloat64(row[76]),
			TrailingReduceSpeedCoefficient:   convertStringToFloat64(row[77]),

			DeferredCheckInterval: convertStringToInt(row[78]),
			DeferredSellInterval:  convertStringToInt(row[79]),

			TotalRevenue:      convertStringToFloat64(row[80]),
			SuccessPercentage: convertStringToFloat64(row[81]),
			PlusRevenue:       convertStringToFloat64(row[82]),
			MinusRevenue:      convertStringToFloat64(row[83]),
			Selection:         convertStringToFloat64(row[84]),
		}
